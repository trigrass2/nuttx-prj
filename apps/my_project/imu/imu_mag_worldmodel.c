...
#include <imu/WorldMagneticModel.h>
#include <imu/WMMInternal.h>
#include <imu/imu.h>


WMMtype_Ellipsoid     Ellip0;
WMMtype_MagneticModel MagneticModel0;
...
bool imu_mag_worldmodel(Imu_Data *imu)
{
	float B[3];
	float decl_deg;
	...
	int month = imu->gps.month;
	int day   = imu->gps.day;
	int year  = imu->gps.year;
	...
	WMM_GetMagVector( imu->gps.latSign * (float)imu->gps.lat,   // [ deg ]
					  imu->gps.lonSign * (float)imu->gps.lon,   // [ deg ]
					  (float)(imu->gps.alt * 0.001),            // [ km ]
					  month,
					  day,
					  year + 2000,
					  &B[0],                                    // [ nT ]
	...
	gWorldMagModel.decl_rad = decl_deg * (float)DEG2RAD;        // [ rad ]

	gWorldMagModel.validSoln      = true;
	gWorldMagModel.timeOfLastSoln = imu->gps.itow;
	return true;
}
...
void WMM_Initialize(void)
{
    ...
    Ellip0.a = 6378.137f;				///< semi-major axis of the ellipsoid in km
    Ellip0.b = 6356.7523142f;			///< semi-minor axis of the ellipsoid in km
    Ellip0.fla = 1 / 298.257223563f;	// flattening
    Ellip0.eps = (float)sqrt(1 - (Ellip0.b*Ellip0.b) / (Ellip0.a*Ellip0.a)); 	///< first eccentricity
    Ellip0.epssq = (Ellip0.eps * Ellip0.eps);									///< first eccentricity squared
    Ellip0.re = 6371.2f;														///< Earth's radius in km

    /// Sets Magnetic Model parameters
    MagneticModel0.nMax       = WMM_MAX_MODEL_DEGREES;
    MagneticModel0.nMaxSecVar = WMM_MAX_SECULAR_VARIATION_MODEL_DEGREES;
    MagneticModel0.SecularVariationUsed = 0;

    // TODO: Really, Really needs to be read from a file - out of date in 2015 at latest
    MagneticModel0.EditionDate = (float)5.7863328170559505e-307;
    MagneticModel0.epoch = 2015.0f;

//    sprintf(MagneticModel0.ModelName, "WMM-2015");

    for (i = 0; i < NUMTERMS; i++)
    {
        MagneticModel0.Main_Field_Coeff_G[i]  = coeffs[i][2];
        MagneticModel0.Main_Field_Coeff_H[i]  = coeffs[i][3];
        MagneticModel0.Secular_Var_Coeff_G[i] = coeffs[i][4];
        MagneticModel0.Secular_Var_Coeff_H[i] = coeffs[i][5];
    }
}
...
* @retval N/A
******************************************************************************/
WMMtype_MagneticModel       TimedMagneticModel0;
WMMtype_GeoMagneticElements GeoMagneticElements0;
WMMtype_CoordSpherical      CoordSpherical0;
WMMtype_CoordGeodetic       CoordGeodetic0;
WMMtype_Date                Date0;
                       ...
                       float*    wmmDecl )
{
    ...
    CoordGeodetic0.lambda = Lon;
    CoordGeodetic0.phi    = Lat;
    CoordGeodetic0.HeightAboveEllipsoid = AltEllipsoid;
    /// geodetic -> Spherical Equations: 17-18, WMM Technical report
    WMM_GeodeticToSpherical( Ellip0,
                             CoordGeodetic0,
                             &CoordSpherical0 );
    ...
    Date0.Day   = Day;
    Date0.Year  = Year;
    WMM_DateToYear(&Date0, Error_Message);

    /// Time adjust the coefficients, Equation 19, WMM Technical report
    WMM_TimelyModifyMagneticModel( Date0,
                                   &MagneticModel0,
                                   &TimedMagneticModel0 );
    ...
    WMM_Geomag( Ellip0,
                ...
                CoordGeodetic0,
                &TimedMagneticModel0,
                &GeoMagneticElements0 );

    *(B+0) = GeoMagneticElements0.X; // North
    *(B+1) = GeoMagneticElements0.Y; // East
    *(B+2) = GeoMagneticElements0.Z; // Vertical
    // GeoMagneticElements.F is the field strength

    *wmmDecl = GeoMagneticElements0.Decl;
}
...
******************************************************************************/
WMMtype_LegendreFunction           LegendreAllocate0;
WMMtype_LegendreFunction           *LegendreFunction0 = &LegendreAllocate0;
WMMtype_SphericalHarmonicVariables SphVariables0;
WMMtype_MagneticResults            MagneticResultsSph0;
WMMtype_MagneticResults            MagneticResultsGeo0;
WMMtype_MagneticResults            MagneticResultsSphVar0;
WMMtype_MagneticResults            MagneticResultsGeoVar0;
                     ...
                     WMMtype_GeoMagneticElements *GeoMagneticElements )
{
    WMM_ComputeSphericalHarmonicVariables( Ellip,
                                           ...
                                           &SphVariables0 ); ///< Compute Spherical Harmonic variables
    WMM_AssociatedLegendreFunction( CoordSpherical,
                                    TimedMagneticModel->nMax,
                                    LegendreFunction0 );    ///< Compute ALF
    WMM_Summation( LegendreFunction0,
                   TimedMagneticModel,
                   SphVariables0,
                   CoordSpherical,
                   &MagneticResultsSph0 ); ///< Accumulate the spherical harmonic coefficients
    WMM_SecVarSummation( LegendreFunction0,
                         TimedMagneticModel,
                         SphVariables0,
                         CoordSpherical,
                         &MagneticResultsSphVar0 ); ///< Sum the Secular Variation Coefficients
    WMM_RotateMagneticVector( CoordSpherical,
                              CoordGeodetic,
                              MagneticResultsSph0,
                              &MagneticResultsGeo0 ); ///< Map the computed Magnetic fields to Geodeitic coordinates
    WMM_RotateMagneticVector( CoordSpherical,
                              CoordGeodetic,
                              MagneticResultsSphVar0,
                              &MagneticResultsGeoVar0 ); ///< Map the secular variation field components to Geodetic coordinates
    WMM_CalculateGeoMagneticElements( &MagneticResultsGeo0,
                                      GeoMagneticElements ); ///< Calculate the Geomagnetic elements, Equation 18 , WMM Technical report
    WMM_CalculateSecularVariation( MagneticResultsGeoVar0,
                                   ...
                                   WMMtype_MagneticResults *MagneticResultsGeo )
{
    ...
    Psi = (float)DEG2RAD * (CoordSpherical.phig - CoordGeodetic.phi);
                                        ...
                                        WMMtype_GeoMagneticElements *MagneticElements )
{
    ...
    MagneticElements->Decldot = (float)RAD2DEG * ( MagneticElements->X * MagneticElements->Ydot -
                                                      MagneticElements->Y * MagneticElements->Xdot ) /
                                                    ( MagneticElements->H * MagneticElements->H );
    MagneticElements->Incldot = (float)RAD2DEG * ( MagneticElements->H * MagneticElements->Zdot -
